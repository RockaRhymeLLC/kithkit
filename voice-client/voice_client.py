#!/usr/bin/env python3
"""
CC4Me Voice Client — runs on a secondary machine (e.g., laptop), listens
for a wake word, captures speech, sends to the CC4Me daemon, plays audio
response.
...
logging.basicConfig(
    level=logging.INFO,
    format="[voice-client] %(asctime)s %(levelname)s %(message)s",
    datefmt="%H:%M:%S",
)
...
def classify_response(text: str) -> str:
    ...
    # (user said something, probably wants to hear it)
    return "confirmed"
...
class CallbackHandler(BaseHTTPRequestHandler):
    ...
    voice_client: "VoiceClient" = None  # type: ignore
...
class CallbackServer:
    ...
    def __init__(self, port: int, voice_client: "VoiceClient"):
        self.port = port
...
class VoiceClient:
    ...
    def _start_ptt_quartz(self):
        ...
        # Map config key names to macOS virtual key codes
        key_codes = {
        ...
        mask = Quartz.kCGEventMaskForAllEvents
    ...
    def _play_audio_with_interrupt(self, wav_data: bytes):
        ...
        If enable_stop_interrupt is True, listens for 'stop' during playback
