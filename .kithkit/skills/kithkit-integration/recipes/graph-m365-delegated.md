...
| `credential-outlook-thehurleys-access-token` | `credential-m365-access-<agent>` | RE-MINT from new refresh token; do NOT copy old value |
...
| `credential-outlook-dave-app-password`, `-outlook-thehurleys-app-password` | (delete) | DELETE — app-password basic auth deprecated |
...
- `credential-outlook-thehurleys-*` is a SEPARATE personal (Hurley) mailbox, not the servos M365 target — listed only to show the access/refresh pattern; scope per your install.
