...
|------|------|-----------|
| **Orchestrator** | `status` (pending→completed/failed), `assignee`, `result`, `error`, `outcome`, `outcome_notes`, `work_notes`, `completed_at` (via status transition) | `acknowledged_at` on Dave-assigned todos ← **guard fires (403)** |
...
| `generate_retro = NULL` (default) | Defers to `config.self_improvement.retro.retro_all_terminal` global |
...
|---------|-----------------|
| Orch sets `acknowledged_at` on a Dave-assigned task | 403 guard fires — only comms sets `acknowledged_at` |
