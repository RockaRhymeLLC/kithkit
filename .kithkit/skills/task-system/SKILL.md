...
|------|------|-----------|
| **Orchestrator** | `status` (pending→completed/failed), `assignee`, `result`, `error`, `outcome`, `outcome_notes`, `work_notes`, `completed_at` (via status transition) | `acknowledged_at` on Dave-assigned todos ← **guard fires (409)** |
