...
- **Read timeout errors in getWebhookInfo**: The daemon is not responding to webhook POSTs — check Cloudflare tunnel → daemon routing.
...
- **Himalaya OAuth hang**: Outlook OAuth tokens expire; re-auth requires interactive browser login (Dave must do this)
