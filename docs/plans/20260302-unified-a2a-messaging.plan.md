...
| 9a | Qualified name (`bmo@relay.example.com`) + `route: "lan"` → INVALID_ROUTE | Unit |
...
| 12 | Peer resolution: qualified name (`bmo@relay.example.com`) → skips config lookup | Unit |
...
### Step 1: Build and merge (KKit-INSTANCE-C first)
...
### Step 3: Deploy to Instance A and Instance B

Since the router lives in core and the extension just wires it:

1. Instance A and Instance B pull latest daemon code
...
**Risk**: Instance A and Instance B have their own extension entry points. They need to:
