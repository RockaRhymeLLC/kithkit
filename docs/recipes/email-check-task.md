...
```bash
curl -X POST http://localhost:3847/api/scheduler/tasks/email-check/run
# → { "status": "ok", "result": "..." }
```
...
- Test the provider connection directly: `curl -X POST http://localhost:3847/api/scheduler/tasks/email-check/run`
