...
- Confirm extension loaded: health response shows `"extension": "<your-agent>"`
...
1. **Never restart the comms agent.** Only the comms agent restarts itself (via `/restart`). Orchestrators and workers must not kill the comms tmux session or write the restart flag file.

2. **Never use `launchctl` for the comms agent plist.** Touching it kills the human's active session.
