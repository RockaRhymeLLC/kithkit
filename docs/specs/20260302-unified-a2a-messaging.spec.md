...
**Repo**: KKit-INSTANCE-C (daemon), kithkit (core — if promoted later)
...
- [ ] **MF-03: Peer name resolution**: The `to` field accepts a bare peer name (e.g., `"bmo"`) for peers in config, or a qualified name (`"bmo@relay.example.com"`) for relay-only peers not in local config.
...
- **Peer name resolution**: Bare names (e.g., `"bmo"`) are resolved against `agent-comms.peers` config for LAN, and optionally qualified with `@relayHost` for relay. Qualified names (e.g., `"bmo@relay.example.com"`) bypass LAN lookup and go straight to relay.
...
- [ ] Should the unified endpoint live in the `kithkit` core (available to all agents) or start in `KKit-INSTANCE-C` and promote later? Recommendation: implement in `KKit-INSTANCE-C` first, promote to `kithkit` after validation.
