...
interface A2ASendRequest {
  ...
  to?: string;       // Peer name (e.g., "bmo") or qualified name ("bmo@relay.example.com")
...
2. **`route: "lan"`**
   ...
   - **Qualified name** (e.g., `bmo@relay.example.com`): Reject with `400 INVALID_ROUTE` — qualified names are relay-only by definition.
...
- **When**: Caller sends `POST /api/a2a/send {"to":"bmo@relay.example.com","payload":{"type":"text","text":"Hi"},"route":"lan"}`
...
- **Peer name resolution**: Bare names (e.g., `"bmo"`) are resolved against `agent-comms.peers` config: exact case-insensitive match first, then unique prefix match (e.g., `"r2"` → `"r2d2"`). Ambiguous prefix matches return `400 INVALID_TARGET`. Qualified names (e.g., `"bmo@relay.example.com"`) bypass LAN lookup and go straight to relay.
...
- [x] ~~Should the unified endpoint live in the `kithkit` core or start in `KKit-INSTANCE-C`?~~ **Resolved**: Build in `kithkit` public core from day one. All agents share this capability — no reason to start in a private repo.
