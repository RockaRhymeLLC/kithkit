...
## Personal Content — NEVER Upstream

These directories contain instance-specific content that must **never** be copied to cc4me-dev or included in any upstream PR. They exist only in the fork.

| Directory | Content | Why it's personal |
|-----------|---------|-------------------|
| `daemon/public/` | Instance website (HTML, blog, art gallery) | Instance-specific content and portfolio |
| `packages/` | Instance-branded CLI and MCP tools | Instance-specific tools and integrations |
| `services/` | 30+ microservices (API gateway, dev tools, relay) | Instance-specific infrastructure |
| `scripts/game/` | Game engine and world data | Instance-specific project |
| `scripts/art/` | Art generation scripts | Instance-specific project |
| `.claude/state/` | Memory, todos, calendar, contacts | Personal state and PII |

**Upstream-eligible directories** are limited to: `daemon/src/`, `scripts/` (excluding game/art), `.claude/skills/`, `.claude/hooks/`, `docs/`, `voice-client/` (genericized only), and config templates.

If you're unsure whether something belongs upstream, it probably doesn't. The rule is: **if it wouldn't make sense for a stranger's fresh CC4Me install, it doesn't go upstream.**

A pre-push hook in `cc4me-dev` enforces this automatically — see Phase 1 below.
...
- `.claude/skills/email/{agent}-fastmail.md`
- `.claude/skills/email/{agent}-m365.md`
...
### GitHub TOS Compliance (see `github-tos` skill)
- **Max 3 PRs per hour** to external repos — space submissions 20+ minutes apart
- Before starting, verify account: `gh auth status`
- Check rate limit: `gh api /rate_limit`
- If submitting all 6 groups, spread across **at least 2 sessions** (3 per session, 20+ min apart)
- Each PR must have a unique, substantive description (not templated)
