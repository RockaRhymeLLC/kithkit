...
imap.login('redacted@outlook.com', '<app-password>')
imap.select('INBOX')
...
```bash
curl -s -X PATCH "https://graph.microsoft.com/v1.0/users/bmo@example.com/messages/<id>" \
...
```bash
curl -s "https://graph.microsoft.com/v1.0/users/bmo@example.com/mailFolders" \
