...
Use this skill when creating a PR on the public kithkit repo (RockaRhymeLLC/kithkit) from a private instance repo (KKit-INSTANCE-A, KKit-INSTANCE-C, KKit-INSTANCE-B).
...
cp ~/KKit-INSTANCE-A/daemon/src/path/to/file.ts daemon/src/path/to/file.ts
```
...
```bash
gh pr edit <number> --repo RockaRhymeLLC/kithkit --add-reviewer agent-reviewer
```
