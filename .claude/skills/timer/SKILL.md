...
All endpoints on `localhost:<daemon-port>` (default 3847):
...
- **Timer flood**: Never set multiple overlapping timers for the same task. The 30s nag cycle on N timers creates a cascade. Use ONE timer at a time — ack/delete before setting the next.
- **bash sleep**: Forbidden. Blocks the entire session. Use a timer instead.
- **Forgetting to ack**: Always acknowledge timers after handling the follow-up. Unacked timers nag every 30s for 10 minutes.
...
- Timer IDs are UUIDs — use the first 8 chars as shorthand in display
...
- Always quote URLs in curl commands (zsh glob expansion)
