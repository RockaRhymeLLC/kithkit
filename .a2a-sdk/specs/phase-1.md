...
| **Phase 2** | Groups (bounded to 50 members) + group key rotation + fan-out delivery | After Phase 1 proven with Instance A + Instance B |
...
1. **Cloudflare Tunnel** (recommended): Free, zero-config. Agent runs `cloudflared` → gets a `*.trycloudflare.com` URL or custom domain. This is what Instance A and Instance B use.
...
**Mitigation**: Multi-admin (Instance A + Instance B) on separate machines with separate keys. Compromise of one key doesn't compromise the other. Admin broadcasts can be cross-verified (single admin signature is valid but unusual — humans should notice).
...
7. Instance A and Instance B transition from v1 to v2 with zero message loss during the 30-day dual-stack period.
