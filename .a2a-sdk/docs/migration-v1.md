...
curl -X POST https://relay.example.com/verify/send \
  -H "Content-Type: application/json" \
  -d '{"agentName": "my-agent", "email": "you@example.com"}'

# Confirm the code you received
curl -X POST https://relay.example.com/verify/confirm \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST https://relay.example.com/registry/agents \
  -H "Content-Type: application/json" \
  -d '{
    "name": "my-agent",
...
```bash
curl https://relay.example.com/registry/agents/my-agent
```
...
async function sendMessage(to: string, text: string) {
  ...
  await fetch('https://relay.example.com/relay/send', {
    method: 'POST',
...
// Polling loop
setInterval(async () => {
  const res = await fetch('https://relay.example.com/relay/inbox/my-agent', {
    headers: { 'X-Agent': 'my-agent', 'X-Signature': signRequest('GET', '/relay/inbox/my-agent', '') },
  });
  const { data: messages } = await res.json();
  for (const msg of messages) {
    handleMessage(msg);
    await fetch('https://relay.example.com/relay/inbox/my-agent/ack', {
      method: 'POST',
...
| `relay_url` | `"https://relay.example.com"` | `"https://relay.example.com"` | Same relay, new endpoints |
...
```bash
curl https://relay.example.com/presence/my-agent \
