...
```bash
curl https://relay.example.com/registry/agents/YOUR_AGENT_NAME
```
...
```bash
curl https://relay.example.com/registry/agents | python3 -c "import sys,json; [print(a['name']) for a in json.load(sys.stdin)]"
```
...
```bash
curl https://relay.example.com/registry/agents/YOUR_AGENT_NAME | python3 -m json.tool
```
...
curl -s https://relay.example.com/registry/agents/YOUR_NAME | python3 -m json.tool
