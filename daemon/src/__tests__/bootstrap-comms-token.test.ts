 ...
 *
 * Note on createDedupedMigrationsDir: this Skippy fork has two version-022 migration files
 * which cause fresh-DB tests to fail with SQLITE_CONSTRAINT_PRIMARYKEY. The helper
 * deduplicates them; it is a no-op in the upstream kithkit repo where version numbers
 * are unique.
 */
...
// ── Migration dedup helper ────────────────────────────────────────────────────
// Builds a copy of the migrations directory with version-number duplicates removed
// (keeping the first alphabetically for each version). This is a no-op when there
// are no duplicate version numbers.
//
// Background: this Skippy fork has two version-022 migration files
// (022-LOCAL-calibration-fields and 022-task-source-field). The migration runner
// does not deduplicate within a single run, so fresh-DB tests fail with
// SQLITE_CONSTRAINT_PRIMARYKEY on the second 022. This helper resolves that.
// The helper is safe to carry upstream — it's a no-op when there are no duplicates.

function createDedupedMigrationsDir(targetDir: string): string {
  const migrationsDir = path.join(targetDir, 'migrations');
  fs.mkdirSync(migrationsDir, { recursive: true });
  // Migrations are compiled to ../core/migrations/ relative to __tests__/
  const srcDir = path.resolve(__dirname, '..', 'core', 'migrations');
  ...
  fs.readdirSync(srcDir)
    ...
    .forEach(file => {
      ...
      if (match) {
        ...
        if (!seen.has(version)) {
          seen.add(version);
          fs.copyFileSync(path.join(srcDir, file), path.join(migrationsDir, file));
        }
      }
    });
...
function setupDb(): void {
  ...
  const migrationsDir = createDedupedMigrationsDir(tmpDir);
  _resetDbForTesting();
  openDatabase(tmpDir, path.join(tmpDir, 'test.db'), migrationsDir);
}
