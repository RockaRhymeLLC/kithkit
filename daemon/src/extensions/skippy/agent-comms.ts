...
interface SkippyConfig extends KithkitConfig {
...
// ── State ─────────────────────────────────────────────────────
let _config: SkippyConfig | null = null;
...
export function getPeerByName(name: string): PeerConfig | undefined {
  const peers = (_config as SkippyConfig)?.['agent-comms']?.peers ?? [];
...
export async function sendAgentMessage(
  ...
  const agentComms = (_config as SkippyConfig)['agent-comms'];
...
export function initAgentComms(config: KithkitConfig): void {
  _config = config as SkippyConfig;
  const agentComms = (_config as SkippyConfig)['agent-comms'];
  if (!agentComms?.enabled) {
