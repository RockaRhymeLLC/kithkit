/**
 * Skippy Extension — registers Telegram adapter, agent-comms, and core tasks.
 *
 * This is the main extension entry point for the Skippy kithkit instance.
 * It wires up:
...
import { createSkippyTelegramAdapter, type SkippyTelegramAdapter } from './telegram.js';
import {
  initAgentComms,
  stopAgentComms,
  handleAgentMessage,
...
let _telegramAdapter: SkippyTelegramAdapter | null = null;
...
export const skippyExtension: Extension = {
  name: 'skippy',

  async onInit(config: KithkitConfig, _server: http.Server): Promise<void> {
    log.info('Skippy extension initializing...');
    _config = config;
    ...
    if (!telegramConfig?.enabled) {
      ...
      try {
        _telegramAdapter = await createSkippyTelegramAdapter({
    ...
    registerRoute('/telegram/status', (async (_req, res) => {
      ...
      res.end(JSON.stringify({
        ok: true,
        adapter: 'skippy-telegram',
        mode: 'polling',
      }));
      return true;
    }) as RouteHandler);

    log.info('Skippy extension initialized');
  },
  ...
  async onShutdown(): Promise<void> {
    log.info('Skippy extension shutting down...');
    ...
    log.info('Skippy extension shut down');
  },
};
