...
const log = createLogger('redacted-vip-inbox-watch');
const execFileAsync = promisify(execFile);

const HIMALAYA_BIN = '/opt/homebrew/bin/himalaya';
const ACCOUNT = 'yahoo-redacted-vip';

const STATE_DIR = join(getProjectDir(), '.claude', 'state');
const SEEN_FILE = join(STATE_DIR, 'redacted-vip-inbox-last-check.json');
const LEDGER_FILE = join(STATE_DIR, 'redacted-vip-purchase-ledger.json');
const DAILY_SUMMARY_FILE = join(STATE_DIR, 'redacted-vip-daily-summary.json');
...
async function fetchRecentEmails(): Promise<Envelope[]> {
  try {
    ...
    log.error('Failed to fetch redacted-vip inbox', { error: err.stderr || err.message });
    return [];
  }
}
...
async function run(): Promise<void> {
  ...
  if (emails.length === 0) {
    log.info('No emails fetched from redacted-vip inbox');
    return;
  }
  ...
  if (urgentAlerts.length > 0) {
    const message = `[redacted-vip's Email]\n${urgentAlerts.join('\n')}`;
    const tgAdapter = getTelegramAdapter();
    if (tgAdapter) {
      await tgAdapter.sendDirect(message);
    } else {
      log.warn('No Telegram adapter for redacted-vip alerts');
    }
  }
...
export function register(scheduler: Scheduler): void {
  scheduler.registerHandler('redacted-vip-inbox-watch', async () => {
    await run();
  });
}
