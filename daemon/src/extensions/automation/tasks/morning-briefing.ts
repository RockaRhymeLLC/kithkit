 ...
 */
function formatPlainBriefing(today: string, weather: string, calendar: string, internalCal: string, lookahead: string, todos: string, overnight: string, emailSummary: string, specialDay?: string | null, redacted-vipSummary?: string): string {
  ...
  if (redacted-vipSummary) {
    lines.push('', "redacted-vip's Email:", redacted-vipSummary);
  }

  return lines.join('\n');
}

function gatherredacted-vipSummary(): string {
  const summaryFile = path.join(getProjectDir(), '.claude/state/redacted-vip-daily-summary.json');
  try {
...
async function run(): Promise<void> {
  ...
  const redacted-vipSummary = gatherredacted-vipSummary();
  ...
  if (sessionExists()) {
    const prompt = [
      ...
      '',
      `EMAIL INBOX:\n${emailSummary}`,
      ...(redacted-vipSummary ? ['', `redacted-vip'S EMAIL (Mom's inbox monitoring):\n${redacted-vipSummary}`] : []),
    ].join('\n');
    ...
    const briefing = formatPlainBriefing(today, weather, calendar, internalCal, lookahead, todos, overnight, emailSummary, specialDay, redacted-vipSummary);
