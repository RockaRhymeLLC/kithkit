 ...
 * redacted-vip inbox summary (flat file).
 *
...
function formatPlainBriefing(sections: {
  ...
  redacted-vipSummary?: string;
}): string {
  const { today, weather, calendar, internalCal, lookahead, todos, overnight, emailSummary, specialDay, redacted-vipSummary } = sections;
...
async function run(): Promise<void> {
  ...
  if (sessionExists()) {
    ...
    const briefing = formatPlainBriefing({
      ...
      todos, overnight, emailSummary, specialDay, redacted-vipSummary,
    });
