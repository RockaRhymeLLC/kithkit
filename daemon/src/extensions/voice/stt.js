...
export function isHallucination(text) {
    ...
    if (!cleaned)
        return true;
    if (HALLUCINATION_PATTERNS.has(cleaned))
        return true;
 ...
 */
export function initSTT(projectDir, model = 'small.en', language = 'en') {
...
export function transcribe(wavPath) {
    return new Promise((resolve, reject) => {
        ...
        if (!fs.existsSync(WHISPER_CLI)) {
            return reject(new Error(`whisper-cli not found at ${WHISPER_CLI}. Install via: brew install whisper-cpp`));
        }
        if (!_modelPath) {
            return reject(new Error('STT not initialized — call initSTT() first'));
        }
        if (!fs.existsSync(_modelPath)) {
            return reject(new Error(`Whisper model not found at ${_modelPath}. Download from https://huggingface.co/ggerganov/whisper.cpp`));
        }
        const args = [
            '-m', _modelPath,
            '-f', wavPath,
