 ...
 */
export function registerClient(clientId: string, callbackUrl: string, ip: string): void {
 ...
 */
export function getConnectedClients(): VoiceClient[] {
 ...
 */
export function getPrimaryClient(): VoiceClient | null {
 ...
 */
export function getRegistryStatus(): { connected: boolean; clients: VoiceClient[] } {
  return {
...
// ---------------------------------------------------------------------------
// Daemon -> Client communication (chime, audio push)
// ---------------------------------------------------------------------------

export interface ChimeResponse {
  status: 'confirmed' | 'rejected' | 'timeout' | 'error';
  error?: string;
}
 ...
 */
export function sendChime(text: string, type: string): Promise<ChimeResponse> {
  ...
  return new Promise((resolve) => {
    ...
    execFile('curl', args, { timeout: 20_000 }, (err, stdout, stderr) => {
      ...
      try {
        const response = JSON.parse(stdout) as ChimeResponse;
 ...
 */
export function sendAudioToClient(audioBuffer: Buffer): Promise<boolean> {
  const client = getPrimaryClient();
  if (!client || !client.callbackUrl) return Promise.resolve(false);
