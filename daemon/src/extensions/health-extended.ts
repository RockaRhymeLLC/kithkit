...
import { registerCheck, type CheckResult } from '../core/extended-status.js';
...
function checkProcesses(): CheckResult {
  const details: Record<string, unknown> = {};
...
function checkLogs(config: BmoConfig): CheckResult {
...
function checkPeers(config: BmoConfig): Promise<CheckResult> {
  ...
  const checks = agentComms.peers.map((peer) => {
    return new Promise<{ name: string; ok: boolean; latencyMs: number }>((resolve) => {
      const hosts: string[] = [peer.host];
      if (peer.ip && peer.ip !== peer.host) hosts.push(peer.ip);

      const tryHost = (idx: number): void => {
        ...
        execFile('curl', ['-s', '--connect-timeout', '3', url], { timeout: 6000 }, (err) => {
          ...
          if (err && idx + 1 < hosts.length) { tryHost(idx + 1); return; }
...
function checkStateFiles(): CheckResult {
 ...
 */
export function registerBmoHealthChecks(config: BmoConfig): void {
