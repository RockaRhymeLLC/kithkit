...
import { setScheduler } from '../api/tasks.js';
import { Scheduler } from '../automation/scheduler.js';
import { asBmoConfig } from './config.js';
import { initComms, shutdownComms, createTelegramRouteHandler, createShortcutRouteHandler, } from './comms/index.js';
...
import { initAgentComms, stopAgentComms, handleAgentMessage, sendAgentMessage, broadcastToAllPeers, getConfiguredPeers, getAgentStatus, updatePeerState, } from './comms/agent-comms.js';
...
let _telegramRouteHandler = null;
let _shortcutRouteHandler = null;
async function handleTelegramWebhook(req, res, pathname, searchParams) {
...
async function handleShortcut(req, res, pathname, searchParams) {
...
async function handleAgentP2P(req, res, _pathname, _searchParams) {
    if (req.method !== 'POST')
        return false;
    try {
...
async function handleAgentMessageRoute(req, res, _pathname, _searchParams) {
    if (req.method !== 'POST')
        return false;
    try {
...
async function handleAgentSend(req, res, _pathname, _searchParams) {
    if (req.method !== 'POST')
        return false;
    try {
        const body = await readBody(req);
...
async function handleAgentBroadcast(req, res, _pathname, _searchParams) {
    if (req.method !== 'POST')
        return false;
    try {
        const body = await readBody(req);
        const { type, text, ...extra } = JSON.parse(body);
        if (!type) {
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ error: 'type is required' }));
            return true;
        }
        const result = await broadcastToAllPeers(type, text, extra);
        res.writeHead(result.ok ? 200 : 502, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify(result));
    }
    catch (err) {
        log.error('Agent broadcast endpoint error', { error: err instanceof Error ? err.message : String(err) });
        res.writeHead(400, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: 'Invalid request' }));
    }
    return true;
}
async function handleAgentPeers(req, res, _pathname, _searchParams) {
    if (req.method !== 'GET')
        return false;
...
async function handleAgentStatusEndpoint(req, res, _pathname, _searchParams) {
    if (req.method === 'GET') {
...
async function handleExtendedStatus(req, res, _pathname, _searchParams) {
    if (req.method !== 'GET')
        return false;
    try {
...
async function handleContextApi(req, res, _pathname, _searchParams) {
    if (req.method !== 'GET')
        return false;
...
// ── Health Checks ───────────────────────────────────────────
function registerBmoHealthChecks() {
...
const BMO_TASK_HANDLERS = [
    ...
    'a2a-digest',
    'memory-sync',
    'redacted-vip-inbox-watch',
    'supabase-keep-alive',
    'blog-reminder',
    'weekly-progress-report',
    'bounty-scanner',
];
async function onInit(config, _server) {
    ...
    registerRoute('/agent/broadcast', handleAgentBroadcast);
    registerRoute('/agent/peers', handleAgentPeers);
