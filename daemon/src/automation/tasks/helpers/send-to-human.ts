 ...
 *
 * Background: the #290 token cutover put an X-Agent-Token role gate on
 * /api/send. Scheduler tasks that call /api/send with bare fetch() will
 * 401 silently. Other tasks that already call routeMessage() in-process
 * are unaffected.
 *
 * Design: PREFER the in-process router — no HTTP hop, no credential
 * dependency. Fall back to HTTP /api/send with the boot-scoped 'daemon'
 * token only if the in-process path throws (it shares none of the HTTP
 * listener's failure domain, so this fallback is mostly belt-and-braces).
 *
 * Channel semantics mirror /api/send exactly: `channels` (array) or
 * `channel` (singular) select adapters; `metadata`/`chat_id` merge the
 * same way.
 *
 * Workers and orchestrators must NOT import this helper — they escalate
 * via /api/messages (worker → orchestrator → comms → human), enforced
 * for them by the /api/send role gate.
 */
