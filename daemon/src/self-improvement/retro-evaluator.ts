...
export function shouldTriggerRetro(task: OrchestratorTask & { workers?: WorkerJob[] }): boolean {
  ...
  if (triggers.on_error && hasError) {
    log.debug('[retro] trigger conditions met, spawning retro worker', { taskId: task.id, reason: 'on_error' });
    return true;
  }

  // Check: retry_count > 0
  if (triggers.on_retry && retryCount > 0) {
    log.debug('[retro] trigger conditions met, spawning retro worker', { taskId: task.id, reason: 'on_retry' });
    return true;
  }

  // Check: workers with error status
  if (triggers.on_error && failedWorkers > 0) {
    log.debug('[retro] trigger conditions met, spawning retro worker', { taskId: task.id, reason: 'failed_workers' });
    return true;
  }

  log.debug('[retro] no trigger conditions matched for task', { taskId: task.id, hasError, retryCount, failedWorkers });
  return false;
}
...
export async function spawnRetro(
  ...
  log.debug('[retro] spawning retro worker for task', { taskId: task.id });
  ...
  log.debug('[retro] retro worker spawned', { taskId: task.id, jobId });
